    ...
    Scaffold(
        ...
        Box(modifier = Modifier.padding(padding)) {
            GoogleMap(modifier = Modifier.fillMaxSize(), cameraPositionState = cameraPositionState) {
                Circle(center = centerPoint, radius = 20000.0, fillColor = Color.Red.copy(alpha = 0.2f), strokeColor = Color.Red, strokeWidth = 5f)
                if (uiState is FleetUiState.Success) {
                    (uiState as FleetUiState.Success).vehicles.forEach { vehicle ->
                        Marker(state = MarkerState(position = LatLng(vehicle.latitude, vehicle.longitude)), title = vehicle.vehicleId, snippet = "Hız: ${vehicle.speedKmh} km/h")
                    }
                }
            }
        }
